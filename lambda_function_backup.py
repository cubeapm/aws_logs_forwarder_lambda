...
1. ELB Access Logs (space-separated format, 15+ fields, up to 29+ fields in modern format):
...
# Configuration
LOG_ENDPOINT = os.environ.get("OTEL_COLLECTOR_ENDPOINT", "https://9d99ce02cb9d.ngrok-free.app/api/logs/insert/jsonline?_stream_fields=event.domain&_time_field=timestamp")
...
class ELBAccessLogProcessor:
  ...
  def parse_elb_access_log(line: str) -> Optional[Dict[str, Any]]:
    try:
      ...
      if len(parts) < 15:
        logger.warning(f"Malformed ELB access log line - insufficient fields: {len(parts)}")
        return None
      
      # ELB Access Logs - capture all fields dynamically
      ...
      elb_log = {
        ...
        "timestamp": parts[1],
        "elb": parts[2],
        "client_ip": client_parts[0] if len(client_parts) > 0 else "",
        "client_port": client_parts[1] if len(client_parts) > 1 else "",
        "target_ip": target_parts[0] if len(target_parts) > 0 else "",
        ...
        # Capture additional fields dynamically based on known ALB format
        "target_group_arn": parts[16] if len(parts) > 16 else "",
        "trace_id": parts[17] if len(parts) > 17 else "",
        "domain_name": parts[18] if len(parts) > 18 else "",
        "chosen_cert_arn": parts[19] if len(parts) > 19 else "",
        "matched_rule_priority": parts[20] if len(parts) > 20 else "",
        "request_creation_time": parts[21] if len(parts) > 21 else "",
        "actions_executed": parts[22] if len(parts) > 22 else "",
        "redirect_url": parts[23] if len(parts) > 23 else "",
        "error_reason": parts[24] if len(parts) > 24 else "",
        "target_port_list": parts[25] if len(parts) > 25 else "",
        "target_status_code_list": parts[26] if len(parts) > 26 else "",
        "classification": parts[27] if len(parts) > 27 else "",
        "classification_reason": parts[28] if len(parts) > 28 else ""
      }
      
      # Add any remaining fields as generic fields
      for i in range(29, len(parts)):
